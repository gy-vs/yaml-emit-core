...
| mergeCommonEntries    | `boolean ⎮ number` | `false` | During node construction, extract entries common to multiple sibling mappings into an anchored mapping, referenced from each source mapping with a `<<` merge key. If a number, sets the minimum number of common entries; `true` uses `3`. Has no effect unless the schema supports merge keys, e.g. with the `'yaml-1.1'` schema or the `merge` option. |
